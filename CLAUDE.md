...
- **OscConfig:** enabled, port (default 8765), allowed_sender_ips (default `[]` = allow-all + startup WARNING; normalised to `list[str]` by `__post_init__` to survive malformed TOML), `listen_iface` (pin the listener to one interface **by name**; empty follows `psn_source_iface`, and a station on auto-detect binds every interface. Resolved by `net_utils.resolve_listen_bind`, the receive-side counterpart of `resolve_plane_source_ip` – it differs in one arm, returning the wildcard rather than the auto-detected primary when nothing is configured, because a listener bound to one auto-picked address would silently stop accepting traffic at every other local address. Fails **closed**: a pinned interface with no address leaves the listener stopped, and the observer's `OSC input` plane restarts it when the interface returns. Device-local; edited in the Network Interface Assignment panel)
